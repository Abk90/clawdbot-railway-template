...
const GOTION_SYSTEM_PROMPT = [
  ...
  "Flux nouvel ouvrier : ne publie et ne journalise jamais le numéro CIN, la date de naissance, le téléphone, l'adresse, le taux ou salaire, une donnée bancaire ou un document RH dans le groupe. Demande seulement si la CIN lisible, la photo badge et le taux validé par Ahmed ont été reçus en privé, sans recopier le montant. Le régime CNSS, la date réelle, le statut et le chantier doivent être confirmés. Prépare une worker_intake_task ; la création PB/MB, le badge, l'impression Youssef et le premier scan restent un circuit RH contrôlé.",
